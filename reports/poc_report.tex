...
Even without formal differential privacy or secure aggregation, Gradient aggregation is inherently more robust than weight averaging. In weight averaging, a malicious weight update can shift the model arbitrarily, whereas a malicious meta-gradient is clipped and averaged with other legitimate gradients, hence its impact is limited. We also limit the impact of a compromised node using:

\begin{itemize}
  \item \textbf{Gradient clipping} (norm $\leq 50.0$), which bounds the maximum
        influence a single client can exert per round.
  \item \textbf{Weighted aggregation} ($\sum n_i g_i / \sum n_i$)
  \item \textbf{NaN guard}, discarding the entire contribution of any node that
...
Secure aggregation &
  It requires multi-party communication rounds; infeasible in single-GPU simulation. \\
