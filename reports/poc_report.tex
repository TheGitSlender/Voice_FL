% VoiceFL-MAML School Project Report
...
\fancyhf{}
\fancyhead[L]{\textbf{Progress Report}}
...
\begin{abstract}
We present \textbf{VoiceFL-MAML}, a two-phase federated meta-learning system for
personalised automatic speech recognition of accented English.
Phase~1 establishes a proof-of-concept using FOMAML-ANIL on LibriSpeech.
Phase~2, \emph{FedLoRA-MAML}, deploys true second-order MAML with LoRA adapters
on the L2-ARCTIC corpus, training over 12 federated nodes spanning six
non-native English accent groups.
After 50 rounds the federated meta-initialisation $\theta^*$ matches or outperforms
\texttt{wav2vec2-base-100h} on three of six accent groups,
with a \SI{3.1}{pp} statistically significant improvement for Vietnamese (THV).
Communication cost is reduced \textbf{295$\times$} relative to full Per-FedAvg
by transmitting only 320K LoRA parameters per round instead of 94.5M.
Projections from the convergence curve indicate that with 200 total rounds the system
may approach the performance of the much larger \texttt{wav2vec2-base-960h} baseline
...
The straightforward remedy---collecting labelled accented speech at scale and
...
Standard FedAvg~\cite{mcmahan2017fedavg} averages client weight updates.
\textbf{Per-FedAvg}~\cite{fallah2020perfedavg} instead aggregates client
...
This formulation ensures each client contributes proportionally to its data
volume, not just its participation in the round.
...
The PEFT library is intentionally not used because PEFT's
\verb|get_peft_model()| replaces parameters in a way that is incompatible with
\verb|higher.innerloop_ctx|, which patches parameters functionally for
...
\begin{tikzpicture}[
  ...
  \node[stage] (raw)  at (0, 0)  {L2-ARCTIC WAVs (22kHz)};
  \node[stage] (prep) at (4, 0)  {\texttt{prepare\_l2arctic}};
  \node[stage] (feat) at (8, 0)  {\texttt{features.pt} (16kHz)};
  \node[stage] (task) at (8,-2)  {Task Sampler ($K$-shot)};
  \node[stage] (maml) at (4,-2)  {MAMLEngine ($k$=5 steps)};
  \node[stage] (grad) at (0,-2)  {Meta-gradient $\nabla_{\theta^*}$};
  ...
  \node[note] at (2.0, 0.45) {per speaker};
  \node[note] at (6.0, 0.45) {float32, [-1,1]};
  \node[note] at (8.0,-1.05) {support $\cap$ query $= \emptyset$};
  \node[note] at (4.0,-2.55) {2nd-order CTC};
\end{tikzpicture}
...
gradients (Equation~\ref{eq:perfedavg}).
...
\begin{enumerate}
  \item \textbf{Cohort selection.} The server selects a random cohort of 3 of 12
        nodes (\verb|cohort_fraction: 0.25|).
  ...
  \item \textbf{Local MAML episode.} Each node runs $k=5$ inner-loop steps on its
        support set, evaluates the query CTC loss, and backpropagates through the
        inner loop with the custom differentiable CTC (second-order).
  \item \textbf{Gradient return.} The node clips the gradient to $\ell_2$ norm
        $\leq 50.0$, serialises it, and returns it in \texttt{FitRes} alongside
...
The \emph{adaptation efficiency} is:
\begin{equation}
...
It grew from 1.6\% at round 1 to 43.4\% at round 45.
...
Even without formal differential privacy or secure aggregation, the protocol
limits the impact of a compromised node:

\begin{itemize}
  \item \textbf{Gradient clipping} (norm $\leq 50.0$) bounds the maximum
        influence a single client can exert per round.
  \item \textbf{Weighted aggregation} ($\sum n_i g_i / \sum n_i$) means a node
        with fewer query examples has proportionally less influence.
  \item \textbf{NaN guard} discards the entire contribution of any node that
        returns invalid gradients, preventing a trivial poisoning attack.
  \item \textbf{Gradient aggregation} is inherently more robust than weight
        averaging: a malicious weight update can shift the model arbitrarily,
        whereas a malicious meta-gradient is clipped and averaged with other
...
\begin{table}[H]
\centering
\caption{Security mechanisms not present in this PoC and the rationale.}
\label{tab:not-present}
\begin{tabular}{p{3.8cm} p{8.5cm}}
\toprule
\textbf{Mechanism} & \textbf{Why omitted} \\
\midrule
Differential privacy &
  Opacus is incompatible with \texttt{higher.innerloop\_ctx}; manual DP
  requires per-sample gradient clipping which breaks second-order differentiability. \\
Secure aggregation &
  Requires multi-party communication rounds; infeasible in single-GPU simulation. \\
mTLS / certificates & Plain gRPC sufficient for local container network. \\
Byzantine robustness (Krum, etc.) &
  Out of scope; three-node cohorts already limit attack surface. \\
\bottomrule
\end{tabular}
\end{table}
...
Training required resolving three distinct failure modes before convergence.
Table~\ref{tab:failures} summarises each.
...
Run 5 & wav2vec2-base-100h &
  ...
  \texttt{MAX\_GRAD\_NORM = 20} too tight; model escaping sharp basin &
...
the blank token.
This is a degenerate fixed point of the loss: the blank path has zero alignment
uncertainty, so it is always the maximum-probability path.
The meta-gradient is zero because the inner loop cannot escape this saddle.
...
is negligible.
Meta-learning requires curvature in the loss landscape; a near-optimal
initialisation flattens it.
...
Every step was clipped, meaning the effective update was always the normalised
...
and clipping ceased entirely for the remaining 20 rounds, indicating the model
...
\begin{table}[H]
\centering
\caption{Key training metrics at four checkpoints.}
\label{tab:convergence}
\begin{tabular}{lrrrr}
\toprule
\textbf{Metric} & \textbf{Round 1} & \textbf{Round 25} & \textbf{Round 45} & \textbf{Round 50} \\
\midrule
Query CTC loss      & 58.3   & 16.4   & 9.2    & 9.2 (plateau) \\
Inner loss $k=0$    & 58.6   & 27.9   & 18.0   & --- \\
Inner loss $k=5$    & 57.6   & 17.6   & 10.2   & --- \\
Adaptation eff.     & 1.6\%  & 37.1\% & 43.4\% & $\approx$43\% \\
Grad norm           & 50     & 154    & 20     & $<$20 \\
Clip coefficient    & 0.40   & 0.33   & 1.00   & 1.00 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Convergence plateau.}
Rounds 35--50 show query CTC $\approx 9.2 \pm 1.5$ (sampling noise),
grad norms 17--25 (below clip threshold), and no clipping for 20 consecutive
rounds.
The model has entered a flat basin; additional rounds on the current
configuration will yield diminishing returns without a larger cohort or
higher-quality data.

Figure~\ref{fig:loss-curve} plots the query CTC loss and adaptation efficiency
over rounds.
...
\begin{tikzpicture}[scale=0.90]
  ...
  \draw[->] (0,0) -- (0,6.2)  node[above]{\small CTC loss};

  % Y gridlines and ticks
  \foreach \y/\lab in {0/0, 1/10, 2/20, 3/30, 4/40, 5/50, 6/60} {
    \draw[gray!30] (0,\y) -- (10.5,\y);
    \draw (-0.1,\y) -- (0,\y) node[left]{\scriptsize\lab};
  }
  % X ticks (rounds 0..50, scale x10=1)
  \foreach \x/\lab in {0/0, 2/10, 4/20, 6/30, 8/40, 10/50} {
    \draw (\x,-0.1) -- (\x,0) node[below]{\scriptsize\lab};
  }

  % Loss curve (rounds 1,5,10,15,20,25,30,35,40,45,50 -> y scaled /10)
  \draw[NavyBlue, very thick, smooth] plot coordinates {
    (0.2, 5.83) (1.0, 4.20) (2.0, 3.10) (3.0, 2.40)
    (4.0, 2.00) (5.0, 1.64) (6.0, 1.30) (7.0, 1.05)
    (8.0, 0.95) (9.0, 0.92) (10.0, 0.92)
  };
  ...
  % Plateau annotation
  \draw[ForestGreen, dashed] (8.0,0.92) -- (10.5,0.92)
    node[right, font=\tiny]{plateau $\approx$9.2};

  \node[font=\small, NavyBlue] at (3.0, 5.0)
    {Query CTC loss};
\end{tikzpicture}
\caption{Query CTC loss over 50 training rounds. The dashed vertical line marks
         the round at which the gradient clip threshold was raised from 20 to 50;
...
These are order-of-magnitude estimates, not precise forecasts.
...
\begin{itemize}
  ...
  \item The custom CTC is 220--450$\times$ slower than the native kernel;
        scaling to longer clips requires careful memory management.
  \item No differential privacy or secure aggregation (research PoC only).
  \item Projections to 200 rounds are based on a two-point fit with large uncertainty.
\end{itemize}

\paragraph{Future work.}
\begin{itemize}
  \item Complete the remaining 150 training rounds.
  \item Implement curriculum sampling that over-represents phonologically distant
        accent groups in early cohorts.
  \item Explore higher LoRA rank ($r = 16$ or $r = 32$) for harder accents.
  \item Investigate compatibility paths for differential privacy with
        second-order MAML.
  \item Port the custom CTC dynamic programming to a CUDA kernel to eliminate
        the Python-loop speed penalty.
\end{itemize}
