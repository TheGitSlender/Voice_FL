# VoiceFL-MAML — Reproducibility Guide

Federated meta-learning for personalized accented speech recognition.  
Phase 1: FOMAML + ANIL, LibriSpeech, Flower FL, Wav2Vec2, Docker.  
Phase 2: FedLoRA-MAML, 12 L2-ARCTIC nodes (6 accent groups), true second-order MAML via custom differentiable CTC, 295× communication reduction.  
No differential privacy. No SecAgg. Plain gRPC.

**Phase 2 result:** `evaluation/results/fedlora_maml_l2arctic.json` — Fed\_k5 matches or outperforms `wav2vec2-base-100h` on 3/6 accent groups at 50 rounds; Vietnamese improvement statistically significant (−3.1 pp, non-overlapping 95% CI).
...
.
├── configs/
│   ├── poc.yaml                    # Phase 1 — FOMAML-ANIL hyperparameters
│   ├── l2arctic_lora_poc.yaml      # Phase 2 — FedLoRA-MAML on L2-ARCTIC (primary)
│   ├── vctk_lora_poc.yaml          # Phase 2 — FedLoRA-MAML on VCTK (archived run)
│   └── lora_poc.yaml               # LoRA model defaults
├── ctc/
│   └── differentiable_ctc.py       # custom CTC in pure PyTorch — enables double-backward
├── data/
│   ├── download.py                 # LibriSpeech download + speaker selection (Phase 1)
│   ├── download_l2arctic.py        # L2-ARCTIC download helper
│   ├── features.py                 # extract float32 waveform tensors, delete raw pkl
│   ├── pii_masking.py              # hash speaker IDs, write raw_clips.pkl
│   ├── task_sampler.py             # VoiceTaskSampler — K-shot episode builder
│   ├── prepare_librispeech.py      # Phase 1 node preparation
│   ├── prepare_l2arctic_lora.py    # Phase 2 node preparation (meta-train/val)
│   ├── prepare_l2arctic_eval.py    # Phase 2 meta-test node preparation
│   ├── prepare_vctk.py             # VCTK node preparation (Phase 1 alt)
│   ├── prepare_vctk_lora.py        # VCTK node preparation (Phase 2 archived)
│   ├── meta_split.json             # Phase 1 speaker split (locked)
│   ├── l2arctic_split.json         # Phase 2 speaker split (locked)
│   ├── vctk_split.json             # VCTK speaker split (archived)
│   ├── diagnostics/
│   │   ├── run_gradient_analysis.py
│   │   └── gradient_norms.json     # inner-LR tuning results
│   └── insights/
│       ├── run_insights.py
│       └── *.json                  # domain shift, speaker variance, support sensitivity
├── docker/
│   ├── Dockerfile.node             # pytorch/pytorch base — FL node image
│   ├── Dockerfile.server           # FL server image
│   ├── Dockerfile.train            # standalone training image
│   ├── docker-compose.yml          # Phase 1: 1 server + 5 LibriSpeech nodes
│   ├── docker-compose.l2arctic.yml # Phase 2: 1 server + 12 L2-ARCTIC nodes
│   ├── docker-compose.vctk.yml     # Phase 2 archived: VCTK stack
│   └── docker-compose.train.yml    # centralized training container
├── evaluation/
│   ├── eval_poc.py                 # Phase 1 pass/fail evaluator
│   ├── eval_lora.py                # Phase 2 WER evaluator with bootstrap CI
│   ├── plot_results.py             # result plotting
│   └── results/
│       ├── fedlora_maml_l2arctic.json  # Phase 2 final WER results (6 speakers, 4 baselines)
│       └── fedlora_maml_vctk.json      # archived VCTK results
├── federated/
│   ├── client_maml.py              # Phase 1 Flower client — FOMAML-ANIL
│   ├── client_lora.py              # Phase 2 Flower client — FedLoRA-MAML
│   ├── server_maml.py              # Phase 1 Flower server
│   ├── server_lora.py              # Phase 2 Flower server + round-offset resumption
│   ├── strategy_maml.py            # PerFedAvgStrategy — gradient aggregation, θ* AdamW update
│   ├── run_node.py                 # Phase 1 node entry point
│   └── run_node_lora.py            # Phase 2 node entry point
├── maml/
│   ├── engine.py                   # MAMLEngine — FOMAML, second-order CTC, LoRA-MAML modes
│   ├── meta_train.py               # centralized MAML training loop
│   └── tracking.py                 # MLflow tracker wrapper
├── models/
│   ├── wav2vec2_maml.py            # Phase 1 — Wav2Vec2MAML, ANIL split
│   └── lora_wav2vec2.py            # Phase 2 — LoRAWav2Vec2, layers 6–11, ~320K trainable params
├── notebooks/
│   ├── 01_maml_toy_verify.ipynb    # MAML correctness on synthetic data
│   ├── 02_model_verify.ipynb       # model parameter partition verification
│   └── 03_centralized_train.ipynb  # centralized training walkthrough
├── reports/
│   ├── poc_report.tex              # school project LaTeX report (Phase 1 + 2)
│   ├── references.bib              # bibliography
│   ├── REPORT.md                   # technical markdown report
│   ├── figures/                    # Phase 1 evaluation figures
│   └── analysis/                   # Phase 2 MLflow diagnostic plots
├── scripts/
│   ├── setup_env.sh                # pre-cache HF models (wav2vec2-base-100h + 960h)
│   ├── prepare_data.sh             # Phase 1 data pipeline
│   ├── build.sh                    # Phase 1 Docker build
│   ├── run_poc.sh                  # Phase 1 federated run + eval
│   ├── run_fedlora.sh              # Phase 2: check/build/launch/logs/mlflow subcommands
│   ├── analyse_run.py              # merge MLflow sessions, generate diagnostic plots
│   ├── benchmark_ctc.py            # custom vs native CTC speed benchmark
│   ├── verify_differentiable_ctc.py # 5-test correctness suite for custom CTC
│   └── generate_report_figures.py  # figure generation for LaTeX report
├── tests/
│   ├── conftest.py
│   └── test_differentiable_ctc.py  # pytest suite for custom CTC
├── docs/
